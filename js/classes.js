...
export function rectIntersect(r1, r2) { // Ici on crée les collisions entre carré 
    return !(r2.x >= r1.x + r1.width || // Droite de r2 à gauche de r1
             r2.x + r2.width <= r1.x || // Gauche de r2 à droite de r1
             r2.y >= r1.y + r1.height || // Bas de r2 au dessus de r1
             r2.y + r2.height <= r1.y); // Haut de r2 en dessous de r1
}

export class Entity {  // Classe de base pour toutes les entités du jeu 
...
export class TimeSensitiveEntity extends Entity { // Entité qui réagit au time stop
    constructor(x, y, width, height, color) {
    ...
    update(deltaTime, worldSpeed, player, correction = 1) { // correction pour le time stop
        this.x += this.vx * worldSpeed * correction;
        this.y += this.vy * worldSpeed * correction;
    }
}
...
export class Wall extends Block {
    constructor(x, y, size, direction, type = 'attack') {
        ...
        if (!Wall.image) {
            ...
            Wall.image.src = 'assets/Image/Sprite/mur/mur.png';
        }
    }
    ...
    draw(ctx) {
        ...
        if (this.direction === 'right' || this.direction === 'left') {
            ...
            for (let i = 0; i < numBlocks; i++) {
                ctx.drawImage(Wall.image, this.x + (i * blockSize), this.y, blockSize, blockSize);
            }
        } else {
            const numBlocks = Math.ceil(this.height / blockSize);
            for (let i = 0; i < numBlocks; i++) {
                ctx.drawImage(Wall.image, this.x, this.y + (i * blockSize), blockSize, blockSize);
            }
        }
...
export class Player extends Entity {
    ...
    update(input, blocks, correction = 1) {
        ...
        for (const block of blocks) { // Gestion des collisions horizontales
        ...
        for (const block of blocks) { // Gestion des collisions verticales
            if (rectIntersect(this, block)) {
                if (this.velY > 0) {
        ...
        for (let i = this.trail.length - 1; i >= 0; i--) { // permet de mettre un effet de traînée au joueur
            this.trail[i].alpha -= 0.08 * correction;
